...
SET SQL_SAFE_UPDATES = 0;
UPDATE customers_cleaned
SET temp_date = STR_TO_DATE(signup_date, '%Y-%m-%d');

ALTER TABLE customers_cleaned
DROP COLUMN signup_date;

ALTER TABLE customers_cleaned
RENAME COLUMN temp_date TO signup_date;

-- validate cleaned table
SELECT COUNT(*) FROM customers_cleaned;

SELECT customer_id, COUNT(*) FROM customers_cleaned GROUP BY customer_id HAVING COUNT(*) > 1;

SELECT MIN(signup_date), MAX(signup_date) FROM customers_cleaned;
SELECT * FROM customers_cleaned WHERE signup_date IS NULL;

SHOW CREATE TABLE customers_cleaned;

SELECT email FROM customers_cleaned WHERE email IS NOT NULL AND email NOT LIKE '%@%';

SELECT COUNT(*) FROM customers_cleaned WHERE email = '' OR city = '' OR state = '' OR customer_segment = '';
