...
SELECT supplier_id, LENGTH(supplier_id), HEX(supplier_id)
FROM products_cleaned
WHERE supplier_id IS NOT NULL
AND supplier_id NOT REGEXP '^[0-9]+$';

ALTER TABLE products_cleaned
MODIFY COLUMN supplier_id VARCHAR(50) NULL;
...
SELECT price_flag FROM products_cleaned WHERE unit_cost > list_price;
SELECT product_status FROM products_cleaned GROUP BY product_status;
SELECT
	SUM(product_id = '') AS blank_product_id,
    SUM(product_name = '') AS blank_product_name,
    SUM(category = '') AS blank_category,
    ...
    SUM(category_flag = '') AS blank_category_flag
FROM products_cleaned;
SELECT * FROM products_cleaned WHERE category_flag IS NOT NULL;
